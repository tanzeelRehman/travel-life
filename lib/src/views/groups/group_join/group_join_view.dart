...
class GroupJoinView extends StackedView<GroupJoinViewModel> {
  ...
  Widget builder(
    ...
    return Scaffold(
      bottomNavigationBar: groupJoin == GroupJoin.join ||
              groupJoin == GroupJoin.requestJoin ||
              groupJoin == GroupJoin.accept
          ? Padding(
              padding: EdgeInsets.only(
                  left: 27.w, right: 27.w, bottom: 10.h, top: 10.h),
              child: MainButton(
                buttonText: getReadableGroupJoinType(groupJoin),
                onPressed: groupJoin == GroupJoin.accept && invitedGroup != null
                    ? () {
                        model.onClickAcceptInvite(invitedGroup!.groupMemberId!);
                      }
                    : () {
